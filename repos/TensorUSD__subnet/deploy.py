...
wallet = bt.Wallet("test_cold")
keypair = wallet.get_coldkey()
...
#             os.path.dirname(__file__), "assets", "flipper-v5.json"
#         ),
#         substrate=substrate,
#     )
# else:
...
code = ContractCode.create_from_contract_files(
    metadata_file=os.path.join(
        os.path.dirname(__file__), "flipper", "target", "ink", "flipper.json"
    ),
    wasm_file=os.path.join(
        os.path.dirname(__file__), "flipper", "target", "ink", "flipper.wasm"
    ),
    substrate=substrate,
)
...
contract = code.deploy(
    ...
    value=0,
    upload_code=True,
    gas_limit={"ref_time": 186130981, "proof_size": 26457},
)
...
contract_receipt = contract.exec(
    keypair, "flip", args={}, gas_limit=gas_predit_result.gas_required
)
