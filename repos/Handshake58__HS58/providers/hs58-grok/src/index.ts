...
xAI Grok models via DRAIN payments. OpenAI-compatible API format.
...
Per-token pricing in USDC (${(config.markup - 1) * 100}% markup on xAI base prices).
...
- Models are auto-synced from the xAI API
`);
});
