...
This repository is **off-chain node software** (orchestrator, worker, validator) that connects to **BeamCore**. The coordination service itself is **[BeamCore V2](https://github.com/Beam-Network/beamcore_v2)** — summaries below match that stack.
...
Clients / Validators                Orchestrators
  ...
  └──────┬────────┘
   ...
   metagraph-sync  — Bittensor chain neuron sync → PostgreSQL
```

| Piece | Role |
| ----- | ---- |
| **core-server** | Public HTTP API, control plane, transfer and task lifecycle (your `SUBNET_CORE_URL` / `BEAM_VALIDATOR_*` URLs point here) |
| **orch-gateway** | Orchestrator WebSocket edge; upstream relay to core-server (`ORCH_GATEWAY_URL`) |
| **worker-gateway** | Worker WebSocket edge; task offers and sessions (`BUFFER_URL` — **worker-gateway origin**, not core-server) |
| **ops-scheduler** | PRISM refresh, payment verification, cleanup jobs |
| **metagraph-sync** | Subtensor sync into BeamCore’s database |
...
> **Note:** `BUFFER_URL` / worker env must target the **worker-gateway** base URL. BeamCore HTTP (`SUBNET_CORE_URL`) is orthogonal (registration, REST, evidence POSTs).

For operator-level connection maps and env naming, see BeamCore V2 [README](https://github.com/Beam-Network/beamcore_v2/blob/main/README.md) and `doc/BEAM_CONNECTION_FLOW.md` in that repository.
...
- [BeamCore V2](https://github.com/Beam-Network/beamcore_v2) — API, control plane, gateways, PRISM pipeline
