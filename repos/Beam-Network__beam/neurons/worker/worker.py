...
async def websocket_loop(state: WorkerState):
    ...
    if not state.buffer_url:
        raise RuntimeError("BUFFER_URL is required for worker-gateway transport")

    ws_url = get_ws_url(state.worker_id, state.api_key, state.buffer_url)
...
async def run_worker(state: WorkerState):
    ...
    try:
        ...
        if not WEBSOCKETS_AVAILABLE:
            raise RuntimeError("websockets library is required for worker-gateway transport")
        if not state.buffer_url:
            raise RuntimeError("BUFFER_URL must point to worker-gateway for worker runtime traffic")
