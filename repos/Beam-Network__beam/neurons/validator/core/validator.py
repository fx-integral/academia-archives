...
class Validator:
    ...
    async def _verify_single_subnet_proof(self, proof_data: dict) -> ProofVerificationResult:
        ...
        try:
            ...
            if worker_signature and worker_hotkey:
                ...
                # Future: require signatures once orchestrator signing is implemented end-to-end
    ...
    async def _discover_orchestrators(self) -> None:
        """Discover orchestrators from BeamCore only."""
    ...
    async def _get_persisted_weight_snapshot(
        ...
        """Fetch the latest usable BeamCore weight snapshot."""
        candidate_epochs: List[int] = []
        latest_data_epoch = await self.subnet_core_client.get_latest_data_epoch()
        for candidate in (self.current_epoch, latest_data_epoch, self.current_epoch - 1):
            if candidate is None or candidate < 0 or candidate in candidate_epochs:
                continue
            candidate_epochs.append(candidate)

        last_error: Optional[Exception] = None
        for epoch in candidate_epochs:
            try:
                snapshot = await self.subnet_core_client.get_recommended_weights(epoch)
                uids = snapshot.get("uids", [])
                weights = snapshot.get("weights", [])
                if not uids or len(uids) != len(weights):
                    logger.warning(
                        "BeamCore weight snapshot for epoch %s is empty or malformed",
                        epoch,
                    )
                    continue
                return (
                    list(uids),
                    list(weights),
                    snapshot.get("formula_version", "beamcore_v2"),
                    snapshot.get("params_hash"),
                    snapshot.get("epoch", epoch),
                )
            except Exception as exc:
                last_error = exc
                logger.debug("No BeamCore weight snapshot for epoch %s: %s", epoch, exc)

        if last_error is not None:
            logger.warning(
                "Unable to fetch BeamCore weight snapshots for epochs %s: %s",
                candidate_epochs,
                last_error,
            )
        return None
    ...
    async def _submit_weight_proof(
        ...
        try:
            await self.subnet_core_client.submit_weight_proof(
                ...
                uids=uids,
                weights=weights,
                on_chain_hash=f"0x{block_number:016x}",
                formula_version=formula_version,
                params_hash=params_hash,
            )
        except Exception as e:
    ...
    async def _submit_scores_to_core(self, epoch: int) -> None:
        ...
        for hotkey in all_hotkeys:
            ...
            if pob_stats and pob_stats.total_proofs > 0:
                ...
                verification_rate = pob_stats.verified_count / pob_stats.total_proofs
            else:
        ...
        try:
            ...
            logger.info(
                f"Submitted {len(scores)} orchestrator scores to SubnetCore for epoch {epoch}"
            )
        except Exception as e:
