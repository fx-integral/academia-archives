"""
Reward Manager - Local emission/reward-share accounting (informational only).

Worker payments are intentionally NOT performed by this orchestrator: the
on-chain ALPHA transfer path, BeamCore payment recording, and the retry
queue have been removed. What remains is purely local accounting that lets
dashboards/state report each worker's emission share for the current epoch.
"""
...
class RewardManager:
    """Tracks emissions and computes per-worker reward shares (no payments)."""
    ...
    def distribute_rewards_at_epoch_end(self, workers_values, get_our_emission) -> Dict[str, float]:
        ...
        reporting; no on-chain transfer is performed.
        ...
        logger.info(
            ...
            "(local accounting only — no on-chain payment)"
        )
