...
class Orchestrator:
    ...
    async def _registry_loop(self) -> None:
        """
        Background loop placeholder for registry connectivity lifecycle.

        NOTE: Registration and transport keepalive are owned by `SubnetCoreClient` over
        WebSocket (BeamCore v2 contract). This loop is kept as a no-op for
        background-task lifecycle parity.
        """
        logger.info(
            "Registry loop: registration and transport keepalive handled via WebSocket, skipping HTTP"
        )
    ...
    # (see beamcore_v2/packages/core-server/src/control-plane/transfer-task-guardrail.ts).
    ...
    async def _init_subnet_core_client(self) -> None:
        ...
        if not self.settings.subnet_core_url:
            logger.warning(
                "SUBNET_CORE_URL not configured - data will not be persisted to BeamCore"
            )
            return
    ...
    async def _handle_task_completion_notification(self, message: dict) -> bool:
        ...
        Worker payments are intentionally NOT triggered here — this
        orchestrator no longer pays workers (see plan
        ``remove_orch_payment_machinery_30534c30``).
