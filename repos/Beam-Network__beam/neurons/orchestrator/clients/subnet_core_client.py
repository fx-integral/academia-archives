...
worker payments to the BeamCore service.

Uses orch-gateway WebSocket for real-time orchestrator control-plane traffic.
Uses BeamCore HTTP only for non-WS control-plane APIs.
"""
...
class SubnetCoreClient:
    ...
    - Report worker payment transactions with record_payment; workers submit payment evidence directly
    """
    ...
    async def acknowledge_task_completions(
        ...
        This records task completion state; workers submit payment evidence directly to BeamCore.
    ...
    async def get_worker(self, worker_id: str) -> Dict[str, Any]:
        ...
        (see beamcore_v2/packages/core-server/src/api/routes/workers.ts). The
