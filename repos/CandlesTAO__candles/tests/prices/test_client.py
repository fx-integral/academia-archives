...
class TestPriceClient:
    ...
    async def test_get_price_by_interval_success(self):
        ...
        with aioresponses() as m:
            ...
            pattern = re.compile(
                r"https://data-api\.coindesk\.com/index/cc/v1/historical/hours.*"
            )
            m.get(pattern, payload=mock_response_data)

            client = PriceClient(api_key="test_key", provider="coindesk")
            result = await client.get_price_by_interval(
                "BTC", "2023-01-01T00:00:00Z::hourly"
            )
    ...
    async def test_get_price_by_interval_http_error(self):
        ...
        with aioresponses() as m:
            ...
            with pytest.raises(aiohttp.ClientResponseError):
                await client.get_price_by_interval(
                    "INVALID", "2023-01-01T00:00:00Z::hourly"
                )
    ...
    async def test_get_price_by_interval_daily_interval(self):
        ...
        with aioresponses() as m:
            ...
            pattern = re.compile(
                r"https://data-api\.coindesk\.com/index/cc/v1/historical/days.*"
            )
            m.get(pattern, payload=mock_response_data)

            client = PriceClient(api_key="test_key", provider="coindesk")
            result = await client.get_price_by_interval(
                "BTC", "2023-01-01T00:00:00Z::daily"
            )
    ...
    def test_get_price_by_interval_unsupported_interval(self):
        ...
        with pytest.raises(ValueError, match="Unsupported interval"):
            ...
            asyncio.run(
                client.get_price_by_interval("BTC", "2023-01-01T00:00:00Z::monthly")
            )
...
class TestGetWeeklyCandle:
    ...
    def test_get_weekly_candle_wrong_provider(self, client):
        ...
        with pytest.raises(
            ValueError, match="Weekly candle method only supports CoinDesk provider"
        ):
            import asyncio
