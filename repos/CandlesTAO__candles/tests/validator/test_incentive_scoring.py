...
class TestIncentiveScoringAndSetWeights:
    ...
    def test_update_validator_scores_handles_empty_scores(
    ...
    def test_update_validator_scores_filters_invalid_uids(
        ...
        invalid_scores[99] = {"average_score": 0.5, "prediction_count": 1}
    ...
    async def test_score_and_update_weights_async(
        ...
        with patch.object(validator, "_intervals_to_score") as mock_intervals:
            with patch.object(
                validator, "_load_predictions_for_intervals"
            ) as mock_load:
                with patch.object(
                    validator, "_write_scoring_results_to_file"
                ) as mock_write_file:
                    ...
                    validator.batch_scorer.score_predictions_by_interval = AsyncMock(
                        return_value=expected_results
                    )
                    validator.batch_scorer.get_miner_scores.return_value = {
                        1: {"average_score": 0.5}
                    }
        ...
        with patch.object(validator, "_intervals_to_score") as mock_intervals:
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        validator,
                        "_score_and_update_weights_async",
                        side_effect=mock_score_impl,
                    ) as mock_score:
                        # Run the method
                        await validator._incentive_scoring_and_set_weights_async()
        ...
        with patch.object(validator, "_intervals_to_score") as mock_intervals:
            with patch.object(
                ...
                with patch.object(
                    ...
                    # Mock scoring error
                    validator.batch_scorer.score_predictions_by_interval.side_effect = (
        ...
        with patch("candles.validator.validator.BaseValidatorNeuron.__init__"):
            with patch("candles.validator.validator.JsonValidatorStorage"):
                with patch.object(Validator, "load_state"):
                    ...
                    mock_price_client_class.assert_called_once_with(
                        api_key="test_api_key", provider="coindesk"
                    )
                ...
                with patch.object(Validator, "load_state"):
                    with patch.dict("os.environ", {"COINDESK_API_KEY": "test_api_key"}):
                        ...
                        with patch("candles.prices.client.aiohttp"):
                            ...
                            validator.config = MagicMock()
                            validator.config.immediate = False
                            validator.config.neuron.disable_set_weights = (
                                True  # Prevent actual weight setting
                            )
                            validator.config.offline = True
                            validator.config.mock = (
                                True  # Prevent background task creation
                            )
                            validator.metagraph = MagicMock()
                            ...
                            assert isinstance(
                                validator.batch_scorer, PredictionBatchScorer
                            )
