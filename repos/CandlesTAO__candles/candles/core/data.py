...
class Prediction(CandlesBaseModel):
    ...
    prediction_id: PositiveInt = Field(
        default=None, description="Unique ID that represents a predication. e.g. 1"
    )

    miner_uid: Optional[NonNegativeInt] = Field(
        default=None, description="Unique ID that represents a miner. e.g. 1"
    )
    ...
    interval_id: str = Field(
        description="Unique ID that represents a interval. Format: timestamp::interval, e.g. 1715000000::HOURLY"
    )

    is_closed: bool = False
    closed_date: Optional[datetime] = Field(
        default=None,
        description="The datetime the prediction was closed. e.g. 2021-01-01 00:00:00",
    )
...
class CandlePrediction(Prediction):
    ...
    interval: TimeInterval = Field(
        description="The interval of the prediction. e.g. HOURLY, DAILY, WEEKLY"
    )

    color: Optional[CandleColor] = Field(
        default=None, description="The color of the candle. e.g. RED, GREEN"
    )
    price: Optional[Decimal] = Field(
        default=None, description="The price of the candle. e.g. 100.00"
    )
    confidence: Optional[Decimal] = Field(
        default=None, description="The confidence of the prediction. e.g. 0.95"
    )
