...
class Evaluator(AbstractEvaluator):
    ...
    def __init__(self, settings=None):
        ...
        self._batch_scoring_concurrency = max(
            1, int(self.settings.max_concurrent_evaluations)
        )
        
        logging.info(
            f"Evaluator initialized: max_concurrent_evaluations={self.settings.max_concurrent_evaluations} "
            f"batch_scoring_concurrency={self._batch_scoring_concurrency} "
        ...
        *,
        ...
        async with challenge_sem:
            try:
                ...
                logging.info(
                    f"[Evaluator]   Model answers: {scoring_result.model_answers}"
                )
                ...
                for q_idx, qa in enumerate(challenge.challenge_questions):
                    model_answer = (
                        scoring_result.model_answers[q_idx]
                        if q_idx < len(scoring_result.model_answers)
                        else ""
                    )
                    q_score = (
                        scoring_result.scores[q_idx]
                        if q_idx < len(scoring_result.scores)
                        else 0.0
                    )
                    detail = (
                        scoring_result.details[q_idx]
                        if q_idx < len(scoring_result.details)
                        else {}
                    )
    ...
    async def evaluate(self, tasks: GetChallengesResponse) -> dict:
        async with self._evaluation_sem:
            ...
            for challenge_scores in scored_per_challenge:
