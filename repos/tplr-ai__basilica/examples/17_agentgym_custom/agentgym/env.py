...
async def monitor_memory():
    ...
    logger.info(
        f"Memory monitoring started (threshold: {MEMORY_THRESHOLD*100}%, interval: {MEMORY_CHECK_INTERVAL}s)"
    )
    ...
    while True:
        try:
            ...
            logger.debug(
                f"Memory usage: {usage_ratio*100:.1f}% ({mem_current}/{mem_max} bytes)"
            )
    ...
    async def evaluate_model(request: EvaluatorRequest):
        ...
        try:
            ...
            if "chutes" in request.base_url.lower():
                ...
                api_key = request.api_key or os.environ.get("CHUTES_API_KEY")
                logger.info(
                    f"Using provided API key for {request.base_url} (validation skipped)"
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to import required modules: {e}"
            )
        except Exception as e:
            tb_str = traceback.format_exc()
            logger.error(tb_str)
            raise HTTPException(
                status_code=500, detail=f"Evaluation failed: {e}, {tb_str}"
            )
...
def create_app():
    ...
    try:
        ...
        if ENV_NAME == "sqlgym":
            ...
            os.environ["AGENTENV_SQLGYM_BIRD_PATH"] = (
                "/app/AgentGym/agentenv-sqlgym/bird/"
            )
        if ENV_NAME == "tool":
