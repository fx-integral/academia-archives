 ...
 *   1. Any PR that touches `src/validator/staticAnalysis.js` MUST add at least
 *      one case here (new attack pattern, or new variant of an old one).
 ...
 * review before this probe suite caught them. See:
 *   https://.../render-service-js#static-analysis (README)
 *
 * Run:  node test/adversary.test.mjs
...
for (const { name, expect, src } of cases) {
  const result = staticValidate(src);
