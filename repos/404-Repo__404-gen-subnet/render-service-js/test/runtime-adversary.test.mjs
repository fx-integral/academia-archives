...
} from '../src/validator/threeAllowlist.js';

// ── two entry points, one implementation ────────────────────────────────────
//
// The browser render path uses `SAFE_THREE_BROWSER_BUNDLE` (a string of
// function sources) instead of importing this module; without the test
// below, nothing stops the two from drifting. We materialize the bundle
// here the same way render-pool.js does inside the Puppeteer page, then
// run every probe through BOTH the Node-side proxy AND the bundle-built
// proxy. Divergence between the two is a test failure.
...
function runCase(tc, entry) {
  const { name, expect, src } = tc;
  const { label, proxy } = entry;
...
for (const tc of cases) {
  // Run every case against every safeTHREE entry point (Node-direct and
  // bundle-reconstructed). A case counts as passing only if BOTH paths
  // behave identically and correctly — if either path diverges, we fail
  // loudly with the label so it's obvious which implementation drifted.
