...
class StreamMiner(ABC):
    def __init__(self, config=None, axon=None, wallet=None, subtensor=None):
        ...
        logger.info(self.config)  # TODO: duplicate print?
        ...
        logger.info(f"Attaching forward function to axon.")
    ...
    def run(self):
        ...
        logger.info(f"Miner starting at block: {self.last_epoch_block}")
    ...
    def prompt(self, synapse: StreamPrompting) -> StreamPrompting:
        ...
        async def _prompt(text: str, send: Send):
            ...
            for token in model(input_ids):
                logger.trace(f"appending token: {token}")
                buffer.append(token)
                # If buffer has N tokens, send them back to the client.
                if len(buffer) == N:
                    time.sleep(0.1)
                    joined_buffer = "".join(buffer)
                    logger.debug(f"sedning tokens: {joined_buffer}")
                    await send(
                        {
            ...
            if buffer:
                ...
                logger.trace(f"Streamed tokens: {joined_buffer}")

        message = synapse.messages[0]
        logger.trace(f"message in _prompt: {message}")
        token_streamer = partial(_prompt, message)
        logger.trace(f"token streamer: {token_streamer}")
