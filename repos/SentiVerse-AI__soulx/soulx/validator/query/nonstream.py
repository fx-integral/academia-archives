...
from soulx.core.models.payload_models import ImageResponse, TextResponse
...
from soulx.core.utils import generic_utils

logger = get_logger(__name__)


def get_response_model_by_task_type(task_config: dict) -> type:

    task_type = task_config.get('task_type', '').upper()
    if task_type == 'IMAGE':
        return ImageResponse
    elif task_type == 'TEXT':
        return TextResponse
    else:
        logger.warning(f"Unknown task type: {task_type}, defaulting to ImageResponse")
        return ImageResponse
...
def get_formatted_response(
    response: Response,
    response_model: type,
) -> Union[ImageResponse, TextResponse] | None:
...
def _extract_response(response: Response, response_model: type) -> Union[ImageResponse, TextResponse] | None:
    try:
        json = response.json()
        formatted_response = response_model(**response.json())

        if response_model == ImageResponse:
            if (hasattr(formatted_response, "is_nsfw") and not formatted_response.is_nsfw) or not hasattr(
                ...
                if hasattr(formatted_response, "image_b64"):
                    if not formatted_response.image_b64:
                        return None
        elif response_model == TextResponse:
            if hasattr(formatted_response, "text") and formatted_response.text:
                return formatted_response
            elif hasattr(formatted_response, "content") and formatted_response.content:
                return formatted_response
            elif hasattr(formatted_response, "choices") and formatted_response.choices:
                return formatted_response
            else:
                return None
...
async def query_nonstream(
    ...
    if response_model is None:
        response_model = get_response_model_by_task_type(task_config)
        logger.info(f"Auto-selected response model: {response_model.__name__} for task type: {task_config.get('task_type', 'UNKNOWN')}")
    ...
    try:
        ...
        await utils.adjust_contender_from_result(
            config=config, query_result=query_result, contender=contender, synthetic_query=synthetic_query, payload=payload, is_text_task=(response_model == TextResponse), task_config=task_config
        )
        return False, None
    ...
    if formatted_response is not None:
        ...
        query_result = utility_models.QueryResult(
            ...
            node_id=node_id,
            node_hotkey=contender.node_hotkey,
            response_time=1, #necessary and harmless, `calculate_work` handles it correctly given `volume=0` when scoring a 400 request
            stream_time=None,
            task=contender.task,
            status_code=response.status_code,
            success=False,
