...
from soulx.core.constants import MAIN_PATH
from soulx.core.storage.base_storage import BaseStorage
from soulx.core.storage.utils import check_key, extract_block_number
...
def _read_json(file_path: Path) -> Optional[dict]:
    try:
        if not file_path.exists():
            logging.debug(f"State file not found: {file_path.as_posix()}")
            return None
            
        with file_path.open("r") as f:
            return json.load(f)
    except json.JSONDecodeError:
        logging.warning(f"Found invalid JSON file: {file_path.as_posix()}, deleting it...")
        try:
            file_path.unlink()
            logging.info(f"Successfully deleted invalid JSON file: {file_path.as_posix()}")
        except Exception as e:
            logging.error(f"Failed to delete invalid JSON file {file_path.as_posix()}: {str(e)}")
        return None
    except Exception as e:
        logging.error(f"Error reading file {file_path.as_posix()}: {str(e)}")
        return None
...
class BaseJsonStorage(BaseStorage):
    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        return self.load_data(key=key, prefix="state") or default
        
    def set(self, key: str, value: Any) -> None:

        self.save_data(key=key, data=value, prefix="state")
