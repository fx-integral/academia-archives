...
| "Copy detected" | Model bytes, shard-invariant tensor content, or activation fingerprint matches another miner's submission | Your weights are identical or near-identical to another miner's. Train your own model. Merged LoRA is allowed; the content-hash check includes attention tensors so attention-only LoRA merges do not collide with the base model just because layernorm/MLP tensors stayed unchanged. |
