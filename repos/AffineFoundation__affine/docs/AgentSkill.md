...
2. **Load previous ratings** from `MINER_STATS` DynamoDB table (persisted per hotkey+revision)
   - **Absence decay applied on load**: if a miner hasn't been scored recently, rating decays toward BASE_RATING
...
4. **Distribute weights** by ELO rating rank with decay: `weight = 0.5^(rank-1)`, then normalize
   - **All miners with ELO history retain weight** (including Pareto-filtered and absent miners)
   - Miners only excluded if: no valid scores, never played ELO, or fully decayed to BASE_RATING

#### ELO Absence Decay (NEW — PR #323)

Miners not participating in a round have ratings decayed toward `BASE_RATING` over time:
- **Time-based**: DB stores rating + `elo_last_scored_at` timestamp; decay computed on load
- **Formula**: `rating = BASE + excess × DECAY_RATE^(effective_rounds^POWER)`
- **Grace period**: 2 rounds (~1 hour) before decay starts
- **Accelerating**: gentle at first (87% at 1h), aggressive over time (1.3% at 12h), gone by 18h
- **Snap to BASE**: when excess < 30 points, snaps to exactly 1200 (no floating point dust)
- **One-shot on return**: offline miners get full accumulated decay applied when they return
- **Why**: prevents stale high ratings from dominating when miners are temporarily filtered/offline
...
| `GEOMETRIC_MEAN_EPSILON` | 0.1 | Smoothing to prevent zero collapse (increased from 0.01) |
...
| `ELO_ABSENCE_DECAY_RATE` | 0.95 | Per-round decay rate for absent miners (~5%/round) |
| `ELO_ABSENCE_DECAY_POWER` | 1.4 | Accelerating exponent (guarantees convergence in ~18h) |
